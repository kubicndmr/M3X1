...
TRAINING_LABEL = "wav2vec_all"

device = t.device('cuda' if t.cuda.is_available() else 'cpu')

## Load data
dataset_path = "../dataset_endlich/Dataset_Wav2vec20/"
ops = sorted(os.listdir(dataset_path))
bad_ops = ['OP_008', 'OP_009', 'OP_011', 'OP_013', 'OP_022', 
           'OP_028', 'OP_037', 'OP_040', 'OP_036']
...
# K-Fold training
for i, (train_index, test_index) in enumerate(kf.split(ops)):
    ## Start
    log_root = TRAINING_LABEL + "/"
    log_path = log_root + "training_" + str(i) + "/"
    log_file = log_path + "log.txt" 

    if i == 0 and os.path.exists(log_root):
        sys.exit("This training label already exists :( \n\"{}\""
                .format(TRAINING_LABEL))
    elif i == 0:
        os.mkdir(log_root)
        os.mkdir(log_root + "/code")

        for f in os.listdir("./"):
            if f.endswith(".py"):
                shutil.copyfile(f, log_root + "/code/" + f)
    ...
    ## Split dataset
    trainset = [ops[idx] for idx in train_index]
    testset = [ops[idx] for idx in test_index]
    validset = trainset[int(len(trainset) * hp.train_test_split):]
    trainset = [op for op in trainset if op not in validset]

    h.print_log("\tTrainset\t: {}".format(trainset), log_file)
    h.print_log("\tValidset\t: {}".format(validset), log_file)
    h.print_log("\tTestset\t\t: {}".format(testset), log_file)
